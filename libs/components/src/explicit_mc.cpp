...
namespace components {
  ...
  std::int8_t Ops<std::int8_t>::Negation(const std::int8_t &t) { return !t; }

  std::int8_t Ops<std::int8_t>::Minus(const std::int8_t &t) { return -t; }
  ...
  namespace explic {
    std::generator<std::int8_t> Evaluator::visitDerefExpr (const IR::DerefExpr& r)  {
      for (std::int8_t addr : this->visit (r.getInner ())) {
	co_yield state->readAddr(addr);
      }
    }
    ...
    {
      State nstate(_internal->state);
      for (auto v : _internal->evaluator.evaluate (s.getExpression ())) {
	if (v)
	  co_yield nstate;
    ...
    std::generator<State> Transfer::visitStore (const IR::Store& s) {
      for (auto storee : _internal->evaluator.evaluate (s.getStoree ())) {
	for (auto addr : _internal->evaluator.evaluate (s.getAddress ())) {
	  State nstate(_internal->state);
	  
	  nstate.writeAddr (addr,storee);
	  co_yield nstate;
	}
      }
