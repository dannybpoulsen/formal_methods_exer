...
namespace IR {
  ...
  public:
	...
	os << " / ";
      ...
      else if constexpr (b == BinOp::SDiv) {
	os << " s/ ";
      }
	...
	os << " * ";
      ...
      else if constexpr (b == BinOp::SLEq) {
	os << " s<= ";
      }
	...
	os << " <= ";
      ...
      else if constexpr (b == BinOp::SGEq) {
	os << " s>= ";
      }
  ...
  constexpr auto findExprIndex () {
    return auto_index<0,T,Constant,Register,AddExpr,SubExpr,DivExpr,SDivExpr,MulExpr,LEqExpr,GEqExpr,SLEqExpr,SGEqExpr,EqExpr,NEqExpr,LtExpr,GtExpr,SLtExpr,SGtExpr,NegationExpr,MinusExpr,DerefExpr,Undef> (); 
  }
  ...
  inline Undef::Undef () : Expr(findExprIndex<Undef> ()) {}
  ...
  public:
    ...
    virtual T visitSDivExpr (const SDivExpr& r) = 0;
    virtual T visitMulExpr (const MulExpr& r) = 0;
    virtual T visitLEqExpr (const LEqExpr& r) = 0;
    virtual T visitGEqExpr (const GEqExpr& r) = 0;
    virtual T visitSLEqExpr (const SLEqExpr& r) = 0;
    virtual T visitSGEqExpr (const SGEqExpr& r) = 0;
    ...
    virtual T visitSLtExpr (const SLtExpr& r) = 0;
    virtual T visitSGtExpr (const SGtExpr& r) = 0;
    ...
    virtual T visitUndefExpr (const Undef& r) = 0;
	...
	return visitDivExpr (static_cast<const DivExpr&> (e));
      case findExprIndex<SDivExpr> ():
	return visitSDivExpr (static_cast<const SDivExpr&> (e));
	...
	return visitGEqExpr (static_cast<const GEqExpr&> (e));
      case findExprIndex<SLEqExpr> ():
	return visitSLEqExpr (static_cast<const SLEqExpr&> (e));
      case findExprIndex<SGEqExpr> ():
	return visitSGEqExpr (static_cast<const SGEqExpr&> (e));
	...
	return visitGtExpr (static_cast<const GtExpr&> (e));
      case findExprIndex<SLtExpr> ():
	return visitSLtExpr (static_cast<const SLtExpr&> (e));
      case findExprIndex<SGtExpr> ():
	return visitSGtExpr (static_cast<const SGtExpr&> (e));
	...
	return visitDerefExpr (static_cast<const DerefExpr&> (e));
      case findExprIndex<Undef> ():
	return visitUndefExpr (static_cast<const Undef&> (e));
